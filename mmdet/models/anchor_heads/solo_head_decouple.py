...
class DecoupledSoloHead(nn.Module):
    ...
    def dice_loss(self,input_list, target_list):
        scores_list = []
        #print("----------")
        for i in range(len(input_list)):
            if input_list[i].size(0) == 0:
                continue
            scores = self.dice_loss_helper(input_list[i], target_list[i])
    ...
    def loss(self,
        ...
        ins_num = 0
        for i in range(5):
            _, _, b_h, b_w = mask_preds_x[i].shape
            ...
            mask_preds_x[i] = F.sigmoid(mask_preds_x[i].view(-1, b_h, b_w)[pos_ins_x])
            mask_preds_y[i] = F.sigmoid(mask_preds_y[i].view(-1, b_h, b_w)[pos_ins_y])
            #import IPython 
            #IPython.embed()
        ...
        loss_mask = self.dice_loss(mask_preds_list, mask_targets_list) * self.dice_weight
        return dict(
            loss_cls=loss_cls,
            loss_mask=loss_mask)
        '''
